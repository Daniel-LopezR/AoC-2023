        ...
        for (let j = 0; j < 3 && firstY !== y && firstX !== x; j++) {
            ...
            if(notOutOfBounds(currentY, currentX)){
                ...
                if(sChar.match(validNumber)){
                    aNSum += findWholeNumber(sChar, currentY, currentX);
                }
            }
        }
    }
...
let partNSum = 0;
console.log("Finding all of the part numbers close to a symbol inside the engine schematic and summing them");
...
for (let i = 0; i < lines.length; i++) {
    ...
    for (let j = 0; j < line.length; j++) {
        ...
        if (character.match(validSymbol)) {
            partNSum += checkAdjacent(i, j)
        }
    }
}
console.log(partNSum);